...
export default function App() {
  ...
  const handleNextDay = () => {
    ...
    const d = new Date(year, month - 1, day);
    d.setDate(d.getDate() + 1);
    const nextDateStr = getDateString(d);
    const todayStr = getDateString(new Date());
    // Blokir navigasi ke tanggal setelah hari ini (hemat Firebase reads)
    if (nextDateStr > todayStr) {
      triggerToast("Data untuk tanggal setelah hari ini belum tersedia.", "inf");
      return;
    }
